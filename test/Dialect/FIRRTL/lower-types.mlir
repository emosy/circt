...
firrtl.circuit "PathsWithoutNLAsAreStillUpdated" {
  // CHECK-NEXT: firrtl.hierpath @{{[^ ]+}} [@PathsWithoutNLAsAreStillUpdated::@foo, @Foo::@[[a_sym:.+]]]
  // CHECK-NEXT: firrtl.hierpath @{{[^ ]+}} [@PathsWithoutNLAsAreStillUpdated::@foo, @Foo::@[[b_sym:.+]]]
  firrtl.hierpath @path [@PathsWithoutNLAsAreStillUpdated::@foo, @Foo::@a]

  firrtl.module @Foo() {
    // CHECK:      firrtl.wire sym @[[a_sym]] : !firrtl.uint<1>
    // CHECK-NEXT: firrtl.wire sym @[[b_sym]] : !firrtl.uint<2>
    %a = firrtl.wire sym @a : !firrtl.bundle<a : uint<1>, b: uint<2>>
  }
